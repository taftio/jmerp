...
rem %~dp0 is name of current script directory under NT/2000
rem So we tack a '1' onto the end of it to make it possible to
rem delete off the trailing \ with a unique expression
set MERP_HOME=%~dp01
set MERP_HOME=%MERP_HOME:\1=%
...
java -DMERP_HOME="%MERP_HOME%" -classpath %LOCALCLASSPATH%;%CLASSPATH% com.chaosserver.merp.gui.swing.CharacterGeneratorWizard
